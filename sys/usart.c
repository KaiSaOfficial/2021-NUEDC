...
uint8_t USARTA0_ControlTable[200];
...
static void USARTA0_SendByte(uint8_t uch_byte)
{
	MAP_UART_transmitData(EUSCI_A0_BASE, uch_byte);
}

void USARTA0_SendString(uint8_t *puch_buf)
{
  for (uint16_t i = 0; true; i++)
  {
    USARTA0_SendByte(puch_buf[i]);
    if (puch_buf[i] == 0x3C && puch_buf[i - 1] == 0xC3 && puch_buf[i - 2] == 0x33 && puch_buf[i - 3] == 0xCC)
      break;
	}	
}
